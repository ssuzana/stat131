...
$$\begin{array}{ll}
\operatorname{Pr}(X=1)=0.1, & \operatorname{Pr}(X=2)=0.2 \\
\operatorname{Pr}(X=3)=0.3, & \operatorname{Pr}(X=4)=0.4
\end{array}$$

The value 3 is a median of this distribution because $\operatorname{Pr}(X \leq 3)=0.6$, which is greater than $1 / 2$, and $\operatorname{Pr}(X \geq 3)=0.7$, which is also greater than $1 / 2$. Furthermore, 3 is the unique median of this distribution.

**Example (A Discrete Distribution for Which the Median Is Not Unique).**

Suppose that $X$ has the following discrete distribution:
$$\begin{array}{ll}
\operatorname{Pr}(X=1)=0.1, & \operatorname{Pr}(X=2)=0.4 \\
\operatorname{Pr}(X=3)=0.3, & \operatorname{Pr}(X=4)=0.2
\end{array}$$

Here, $\operatorname{Pr}(X \leq 2)=1 / 2$, and $\operatorname{Pr}(X \geq 3)=1 / 2$. Therefore, every value of $m$ in the closed interval $2 \leq m \leq 3$ will be a median of this distribution. The most popular choice of median of this distribution would be the midpoint 2.5 .
...
Here, for every value of $m$ in the closed interval $1 \leq m \leq 2.5$, $\operatorname{Pr}(X \leq m)=\operatorname{Pr}(X \geq m)=1 / 2$. Therefore, every value of $m$ in the interval $1 \leq m \leq 2.5$ is a median of this distribution.
...
$$M(t) = E(e^{Xt}) = \sum_{k=0}^{\infty} e^{tk}q^kp = p \sum_{k=0}^{\infty} (e^{t}q)^k = \frac{p}{1-e^tq}$$ since $\sum_{k=0}^{\infty} (e^{t}q)^k$ is a geometric series with ratio $qt < 1$.
...
(b) $\operatorname{Pr}(Y \geq 2 \mid X=0.5)$.
